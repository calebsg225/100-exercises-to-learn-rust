...
pub enum TicketNewError {
    TitleError { title_error: String },
    DescriptionError { description_error: String },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        ...
        Err(e) => match e {
            ...
            TicketNewError::DescriptionError { description_error } => {
                println!("{}: default description used.", description_error);
    ...
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError {
                title_error: "Title cannot be empty".to_string(),
            });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {
                title_error: "Title cannot be longer than 50 bytes".to_string(),
            });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError {
                description_error: "Description cannot be empty".to_string(),
            });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError {
                description_error: "Description cannot be longer than 500 bytes".to_string(),
            });
        }
